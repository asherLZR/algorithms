...
while U:
    ...
    # once it has been chosen, its cost returns to infinity
    C[chosen] = float('inf')
    # the vertex has now been chosen so it cannot be picked again
    U.remove(chosen)
    mst_cost += c_cost
    ...
    for u, v, w in edges:
        ...
        if chosen == v:
            u, v = v, u
        # if edge has chosen vertex, it is adjacent
        if chosen == u:
            if v in U and w < C[v]:
